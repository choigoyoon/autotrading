"""
진입 캔들의 High/Low 기준 TP/SL 최적화
- SL: 진입 캔들 Low (또는 HL) 이탈
- TP: 진입 캔들 High 기준 배수
"""
...
print("="*80)
print("진입 캔들 High/Low 기준 TP/SL 최적화")
print("="*80)
...
for idx, signal in signals.iterrows():
    ...
    # 진입 캔들 찾기
    candle_idx = df_15m[df_15m['datetime'] == entry_time]
    
    if len(candle_idx) > 0:
        candle = candle_idx.iloc[0]
        entry_candle_info.append({
            'idx': idx,
...
entry_df = pd.DataFrame(entry_candle_info)
signals_with_candle = signals.merge(entry_df, left_index=True, right_on='idx')

print(f"\n시그널 수: {len(signals_with_candle)}")

# 진입 캔들 통계
print("\n진입 캔들 통계:")
print(f"  High - Entry: 평균 {(signals_with_candle['candle_high'] - signals_with_candle['entry_price']).mean()/signals_with_candle['entry_price'].mean()*100:.2f}%")
print(f"  Entry - Low: 평균 {(signals_with_candle['entry_price'] - signals_with_candle['candle_low']).mean()/signals_with_candle['entry_price'].mean()*100:.2f}%")
print(f"  Entry - HL: 평균 {(signals_with_candle['entry_price'] - signals_with_candle['hl_price']).mean()/signals_with_candle['entry_price'].mean()*100:.2f}%")
...
def run_backtest_entry_candle(signals_df, signals_with_candle, future_cache,
                                tp_multiplier, sl_type, sl_buffer_pct,
                                use_trailing, max_hold_hours, split_ratio=0.5):
    """
    진입 캔들 기준 백테스트
    
    tp_multiplier: 진입캔들 고점-진입가 거리의 배수 (1.0 = 고점, 2.0 = 2배)
    sl_type: 'hl' (HL 가격) or 'candle_low' (진입캔들 저점)
    sl_buffer_pct: SL 가격 아래 버퍼 %
    """
    ...
    for idx, signal in signals_df.iterrows():
        ...
        # 캔들 정보 찾기
        candle_info = signals_with_candle[signals_with_candle['idx'] == idx]
        if len(candle_info) == 0:
            continue
        
        candle_info = candle_info.iloc[0]
        
        entry_price = signal['breakout_price']
        candle_high = candle_info['candle_high']
        candle_low = candle_info['candle_low']
        hl_price = signal['hl_price']
        
        # TP 목표: 진입가 + (고점-진입가) * 배수
        tp_distance = candle_high - entry_price
        if tp_distance <= 0:
            tp_distance = entry_price * 0.01  # 최소 1%
        
        tp1_target = entry_price + tp_distance * tp_multiplier * 0.5  # 절반
        tp2_target = entry_price + tp_distance * tp_multiplier  # 전체
...
tp_multipliers = [1.0, 1.5, 2.0, 3.0, 4.0, 5.0]
            ...
            for trailing in trailing_options:
                for max_hold in max_hold_options:
                    for split in split_options:
                        ...
                        df = run_backtest_entry_candle(
                            signals, signals_with_candle, future_cache,
...
for i, row in opt_df.head(15).iterrows():
    ...
    print(f"  리스크: {row['avg_risk']:.2f}% | 리워드: {row['avg_reward']:.2f}%")
...
for i, row in opt_df.sort_values('win_rate', ascending=False).head(10).iterrows():
    print(f"\nTP: {row['tp_multiplier']:.1f}x | SL: {row['sl_type']} | 승률: {row['win_rate']:.1f}% | 평균: {row['avg_pnl']:+.2f}%")
...
print(f"\n   === 리스크/리워드 ===")
print(f"   평균 리스크: {best['avg_risk']:.2f}%")
print(f"   평균 리워드: {best['avg_reward']:.2f}%")
print(f"   R:R 비율: 1:{best['avg_reward']/best['avg_risk']:.1f}")
...
print(f"  - 평균 수익: ~+0.60%")
print(f"\n최적화 전략:")
print(f"  - TP1 달성률: {best['tp1_rate']:.1f}% ({best['tp1_rate']/17.3*100-100:+.0f}%)")
print(f"  - 승률: {best['win_rate']:.1f}% ({best['win_rate']/26.4*100-100:+.0f}%)")
print(f"  - 평균 수익: {best['avg_pnl']:+.2f}% ({(best['avg_pnl']/0.6-1)*100:+.0f}%)")
