...
핵심 아이디어:
- 기존: HL -1%에서 손절
- 개선: EMA 200 이탈 시 손절 (추세 전환 시 빠른 탈출)
...
for i in range(EMA_PERIOD + 100, len(candles_df)):
    ...
    if position is not None:
        ...
        # 2. EMA 이탈 SL (종가가 EMA 아래로 떨어지면)
        if candle['close'] < ema_value:
            # 진입 시 EMA 위였다면
            if position.get('was_above_ema', False):
...
print(f"\n📊 손절 통계:")
print(f"  HL 기반 SL: {sl_by_hl}회")
print(f"  EMA 이탈 SL: {sl_by_ema}회")
...
if len(trades_df) > 0:
    ...
    print(f"\n{'지표':<15} {'기존 HL전략':<20} {'EMA 손절 추가':<20} {'차이':<15}")
    print("-"*70)
    ...
    trades_df.to_csv('backtest_ema_sl_results.csv', index=False)
    print(f"\n✅ 결과 저장 완료")
