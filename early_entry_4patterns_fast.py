...
print("=" * 70)
print("4-패턴 조기 진입 전략 통합 백테스트 (최적화)")
print("=" * 70)
print()
...
print(f"  돌파 인덱스: {len(breakout_indices)}개")

# 각 인덱스에서 5봉 이내 돌파 여부 미리 계산
...
for l in l_values[::3]:  # 샘플링
    ...
    # near_breakout 체크
    if not df.iloc[bounce_idx]['near_breakout']:
        continue
...
print(f"  거래: {len(pattern_trades['L-bounce'])}개")
...
for i in range(50, len(df) - 50):
    ...
    # near_breakout 필터 (빠른 스킵)
    if not row['near_breakout']:
        continue
...
print(f"  거래: {len(pattern_trades['RSI-oversold'])}개")
...
print(f"  거래: {len(pattern_trades['BB-lower'])}개")
...
print(f"  거래: {len(pattern_trades['EMA50-dev'])}개")
print()
...
for pattern_name, trades in pattern_trades.items():
    if len(trades) > 0:
        ...
        ratio = len(trades) / len(all_trades) * 100 if len(all_trades) > 0 else 0
...
if len(all_trades) > 0:
    ...
    print("4. 추세선 돌파 5봉 이내 필터로 거짓 신호 제거")
