        ...
        if candle['close'] < trendline_price and candle['low'] < trendline_price * 0.997:
            ...
            if lh_found:
                short_signals.append({
                    'l1_time': l1_time.strftime('%Y-%m-%d %H:%M:%S'),
                    'l1_price': l1_price,
                    'l2_time': l2_time.strftime('%Y-%m-%d %H:%M:%S'),
                    'l2_price': l2_price,
                    'breakout_time': candle['datetime'].strftime('%Y-%m-%d %H:%M:%S'),
                    'breakout_price': candle['close'],
                    'trendline_price': trendline_price,
                    'gap_pct': gap_pct,
                    'lh_confirm_time': lh_time.strftime('%Y-%m-%d %H:%M:%S'),
                    'lh_price': lh_price,
                    'signal_type': 'SHORT'
                })
            break
...
n_short = max(int(len(short_signals_df) * 0.10), 10) if len(short_signals_df) > 0 else 0
...
print(f"   숏 신호: {len(selected_short)} / {len(short_signals_df)} (10%)")
...
# ✅ 핵심 수정: breakout_time을 datetime으로 변환
...
for idx, signal in all_signals.iterrows():
    ...
    for ex_idx, candle in exit_candles.iterrows():
        ...
        if signal_type == 'LONG':
            ...
            # 추세선 마감 손절
            hours_since = (candle['datetime'] - pd.to_datetime(signal['l2_time'])).total_seconds() / 3600
            l1_price = signal['h1_price'] if 'h1_price' in signal else signal['l1_price']
            l2_price = signal['h2_price'] if 'h2_price' in signal else signal['l2_price']
            l1_time = pd.to_datetime(signal['h1_time'] if 'h1_time' in signal else signal['l1_time'])
            l2_time = pd.to_datetime(signal['h2_time'] if 'h2_time' in signal else signal['l2_time'])
            
            slope = (l2_price - l1_price) / ((l2_time - l1_time).total_seconds() / 3600)
            trendline_price = l2_price + slope * hours_since
        ...
        # 숏 포지션
            ...
            # 추세선 마감 손절
            hours_since = (candle['datetime'] - pd.to_datetime(signal['l2_time'])).total_seconds() / 3600
            l1_price = signal['l1_price']
            l2_price = signal['l2_price']
            l1_time = pd.to_datetime(signal['l1_time'])
            l2_time = pd.to_datetime(signal['l2_time'])
...
print(f"롱 거래: {len(results_df[results_df['signal_type'] == 'LONG'])}")
print(f"숏 거래: {len(results_df[results_df['signal_type'] == 'SHORT'])}")
