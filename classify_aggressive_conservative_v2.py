...
print(f"\n24h Trend (선행 추세):")
print(f"  평균: {features_df['trend_24h'].mean():+.2f}%")
print(f"  중앙값: {features_df['trend_24h'].median():+.2f}%")
print(f"  75%: {features_df['trend_24h'].quantile(0.75):+.2f}%")
...
aggressive_1 = features_df[
    ...
    (features_df['trend_24h'] >= features_df['trend_24h'].quantile(0.75))
]
...
aggressive_2 = features_df[
    ...
    (features_df['trend_24h'] >= 0)
]

# 시나리오 3: 유연한 기준 (3개 이상 조건 만족)
conditions = [
    features_df['gap_pct'] >= features_df['gap_pct'].quantile(0.75),
    features_df['volume_ratio'] >= features_df['volume_ratio'].quantile(0.75),
    features_df['trend_24h'] >= features_df['trend_24h'].quantile(0.75),
...
for scenario_num, aggressive_indices in enumerate([aggressive_1, aggressive_2, aggressive_3], 1):
    ...
    if len(aggressive_trades) > 0:
        agg_win = (aggressive_trades['pnl'] > 0).sum() / len(aggressive_trades) * 100
        agg_avg = aggressive_trades['pnl'].mean()
    ...
    if len(conservative_trades) > 0:
        con_win = (conservative_trades['pnl'] > 0).sum() / len(conservative_trades) * 100
        con_avg = conservative_trades['pnl'].mean()
...
signals_classified = signals.merge(
    features_df[['breakout_time', 'gap_pct', 'body_ratio', 'volume_ratio', 'trend_24h', 'bullish_ratio', 'entry_style']],
    on='breakout_time',
    suffixes=('_orig', '_calc')
)

# Gap 컬럼 충돌 해결
if 'gap_pct_calc' in signals_classified.columns:
    signals_classified['gap_pct'] = signals_classified['gap_pct_calc']
    signals_classified = signals_classified.drop(columns=['gap_pct_orig', 'gap_pct_calc'])
...
print("🎯 거래 전략 제안")
print(f"{'=' * 60}")
print(f"\n공격적 포지션 ({len(signals_classified[signals_classified['entry_style'] == 'AGGRESSIVE'])}개):")
print(f"  • 레버리지: 3x ~ 5x")
print(f"  • 진입 비중: 전체 자본의 30-40%")
print(f"  • 특징: 강한 돌파 + 거래량 + 상승 추세")

print(f"\n보수적 포지션 ({len(signals_classified[signals_classified['entry_style'] == 'CONSERVATIVE'])}개):")
print(f"  • 레버리지: 1x ~ 2x")
print(f"  • 진입 비중: 전체 자본의 10-20%")
print(f"  • 특징: 약한 돌파 또는 하락 추세")
