...
def fetch_doge_cryptocompare(days=1825):
    """
    CryptoCompare에서 DOGE 데이터 수집
    """
    ...
    # 2000개씩 요청 (최대 제한)
    for batch in range(1):  # 1번 요청 (2000일)
        params = {
            'fsym': 'DOGE',
            'tsym': 'USD',
            'limit': 2000,
            'toTs': int(time.time()) - (batch * 2000 * 86400)
        }
        ...
        try:
            ...
            if data['Response'] != 'Success':
                print(f"  오류: {data.get('Message', '알 수 없음')}")
                break
...
# DOGE 데이터 수집
df = fetch_doge_cryptocompare(days=1825)
